...
import { AudioChunkItem, SrtLine } from '../types'
...
export const trimTrailingSilence = (
  ...
  if (potentialSilentSamples < minSilenceSamples) {
    console.log(
      `[Trim Trailing Silence] Trailing silence too short (${(potentialSilentSamples / sampleRate).toFixed(2)}s < ${minSilenceDuration}s), skipping trim`
    )
    return audioBuffer
  }
  ...
  console.log(
    `[Trim Trailing Silence] Original: ${originalDuration.toFixed(2)}s, Trimmed: ${trimmedDuration.toFixed(2)}s, Removed: ${removedDuration.toFixed(2)}s (${((removedDuration / originalDuration) * 100).toFixed(1)}%)`
  )
...
export const splitTextIntoChunks = (
  ...
  for (const sentence of rawSentences) {
    ...
    // If a single sentence is already longer than maxLength or exceeds max time on its own, we have to split it by words
    // (Note: Splitting strictly by words won't preserve perfect sentence boundaries, but is a necessary fallback for extreme cases)
    if (trimmedSentence.length > maxLength || sentenceEstimatedSeconds > maxEstimatedSeconds) {
      ...
      let wordBuffer = ''
      let wordBufferTime = 0

      for (const word of words) {
        const space = wordBuffer ? ' ' : ''
        const newWordBuffer = wordBuffer + space + word
        const newWordTime = newWordBuffer.replace(/\s/g, '').length * 0.156

        if (newWordBuffer.length <= maxLength && newWordTime <= maxEstimatedSeconds) {
          wordBuffer = newWordBuffer
          wordBufferTime = newWordTime
        } else {
          if (wordBuffer.length > 0) finalChunks.push(wordBuffer)
          wordBuffer = word
          wordBufferTime = word.replace(/\s/g, '').length * 0.156
        }
      }
      if (wordBuffer.length > 0) {
        currentChunk = wordBuffer
        currentLineCount = 1 // Word buffers are treated as single line parts
        currentEstimatedSeconds = wordBufferTime
      }
    } else {
      // Check if adding this exceeds length, line count, or time limits
...
const blobToBase64 = async (blob: Blob): Promise<string> => {
  ...
  try {
    ...
    console.log(
      '[blobToBase64] Success! Base64 length:',
      base64.length,
      'compression ratio:',
      ((base64.length / blob.size) * 100).toFixed(1) + '%'
    )
    ...
    throw new Error(
      `Failed to convert blob to base64: ${error instanceof Error ? error.message : 'Unknown error'}`
    )
  }
}
...
export function spliceAudio(
...
): { newBuffer: AudioBuffer; newSrtLines: SrtLine[] } {
  ...
  const segmentsToKeep = editedLines
    .map(editedLine => {
      ...
      return {
        ...
        endSample: Math.floor(
          (Math.min(originalBuffer.duration * 1000, endTimeMs) / 1000) * sampleRate
        ),
  ...
  if (totalLength <= 0)
    throw new Error(
      '편집 후 오디오가 비어 있습니다. 모든 텍스트가 삭제되었거나 타임코드가 유효하지 않습니다.'
    )
